...
\end{split}
\end{equation} 
We want to combine the two remaining exponential functions. In order to achieve this, we first introduce $\hat{F}(t)
=
 e^{-i\int_{-\infty}^{t}\mathrm{d}t^{\prime} \hat{V}_{in}(t^{\prime})}
...
To merge both exponential functions, the BCH-formula must be applied. Note that, due to the chronological time ordering, we could set the involving commutator to zero $ T[] = 0$. Hence
