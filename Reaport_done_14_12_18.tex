...
\end{equation}
This product is called the scattering operator $ \hat{S} $. It connects any state 
\begin{equation}
\ket{in}=\hat{S}\ket{out}.
\end{equation}
By taken $ t $ to the initial conditions of the pictures we obtain two more expressions. Which we will use to verify explicit expressions for $ \hat{S} $. 
...
\end{subequations}
Let's consider the first equality of \eqref{S_defi} by inserting the expressions given in Eq.\enskip\eqref{eq:Omega_in_converg} and \eqref{eq:Omega_out_converg},
\begin{equation}
...
We see the overlap makes $ \hat{S} $ time-independent. Therefore $ t $  is set to  $ \infty $:
...
=\hat{\Omega}_{out}(-\infty)^{-1}.
\end{equation}
%Eq.\enskip\eqref{eq:S_equal_in_infty} is very similar to the definition in Eq.\enskip\eqref{eq:S_in_out} for $ \hat{\Omega}_{I} $. This comes from $ \hat{V}_{I} $ and $ \hat{V}_{in} $ both being picture transformations of $ \hat{V}_{H} $ in the Heisenberg picture. We discussed the main difference lies in position of the condition on the time span. T
The time independence and integral over all $ t $ makes them equal. 
\begin{equation}\label{eq:S_equal_in_plus_minus_infty}
\textcolor[RGB]{0,0,204}{
\hat{S}=T\left( e^{-i\int_{-\infty}^{\infty}\mathrm{d}t \hat{V}_{I}(t)}\right)
.}
\end{equation}
The next set of relations for $ \hat{S} $ to be verified are:
\begin{subequations}
\textcolor[RGB]{0,0,204}{
\begin{align}
...
\end{subequations}
The first one on the right hand side is just $ 1 $ seen directly by the  definition $ \hat{S} = \hat{\Omega}_{I}(\infty) $.
...
\end{align}
\end{subequations}
This last step required $ \hat{S} $ to be unitary. This follows directly as in the expression in Eq.\enskip\eqref{S_all_3_exp} only unitary operators have been used.
\subsection{Wick theorem and vacuum stability}\label{Vacuum_transition}
The Wick theorem as a means to evaluate correlation functions, can be established by further investigating $ \hat{S} $. We choose $ \sum_{\underline{k}}\int dt \ \bar{j_{\underline{k}}}(t) \hat{q}_{\underline{k},in}(t) $ in the chronological time-ordered exponential function of $ \hat{S} $ and using expressions found in section\enskip\ref{in_out_picture_external_currents}.\\
Explicitly,
...
\begin{subequations}
\begin{align}
p_{0,k}&=\left| \braket{0_{out}|0_{in}}\right|^{2}=\left| \braket{0_{in}|\hat{S}|0_{in}}\right|^{2}
	&\\
...
\end{equation}
Expressing $ \hat{\Omega}_{in} $ by $ \hat{S} $ as seen above:
\begin{equation}
Q_{H}(t)
=\left( 
\bar{T}
...
\end{align}
\end{subequations}
This step used the unitarity of $ \hat{S} $ itself and the appendix for transition to chronological time ordering. Following that it is allowed to merge $ T $ due to zero overlap in the boundaries and correct order
\begin{equation}
%%
Q_{H}(t)
=\hat{S}^{-1}
...
\end{align}
\end{subequations}
Identifying the exponential function as $ \hat{S} $, the result is:
\begin{equation}\textcolor[RGB]{0,0,204}{
Q_{H}(t)=
\hat{S}^{-1}
T\left( 
...
\right)
\end{align}
\text{,inserting the expression with $ \hat{S} $ for a $ \hat{\Omega} $ depending on $ t_{j} $  ,}
\begin{align}
&=
T\left( 
\prod_{j}
...
\right).
\end{equation}
Moving $ \hat{S} $ outside of $ T $ gives the final form of the Gell-Mann Low formula:
...
\end{equation}
To proof $ \hat{S} $ can be outside of chronological time ordering we look at $ T(\hat{S}^{-1}) $:
\begin{subequations}
\begin{align}
 T({\hat{S}^{-1}})
 &= T(\hat{S}^{\dagger})
