...
.}
\end{equation}
A generalized expression containing $ n $ $ \hat{V} $ can be formulated from appendix \ref{chronological_time}.
...
In section \ref{in_out_picture_external_currents} we saw that the creation and annihilation operators associated with the $ in $ and $ out $ pictures and each other in Eq.\enskip\eqref{differ_by_current} via an external current. 
We wish to establish how this connection manifolds at the level of the corresponding interaction states. To this end, we particularize Eq.\enskip\eqref{eq:state_interac_schrodinger} to the case in which the initial condition is taken at $ t \rightarrow \pm \infty $.
...
\end{equation}
We will verify that it is explicit time independent by inserting the expressions given in Eq.\enskip\eqref{eq:Omega_in_converg} and \eqref{eq:Omega_out_converg},
...
\right]^{-1}	
	.
\end{equation}
As we stated in footnote $(1)$, $ \hat{\Omega}_{I}(t) $ is a unitary operator. This property extends to $ \hat{\Omega}_{in}(t) $ and $ \hat{\Omega}_{out}(t) $. So, $ \hat{\Omega}_{out}(t)^{-1}=\hat{\Omega}_{out}(t)^{\dagger} $. Therefore:
\begin{equation}
\hat{S}=T\left( e^{-i\int_{-\infty}^{t}\mathrm{d}t^{\prime} \hat{V}_{in}(t^{\prime})} \right)
	\cdot
	\left[ 
...
\right]^{\dagger}
	.
\end{equation}
The hermitian conjugation of anti-chronological time ordering of operators $ \hat{V}(t)$ turns it into the chronological time ordering of same operators, as long as they are hermitian. Noted that this can be seen immediately by hermitian conjugate Eq.\enskip\eqref{eq:anti-chron-time_ordering}
...
\hat{S}=T\left( e^{-i\int_{-\infty}^{t}\mathrm{d}t^{\prime} \hat{V}_{in}(t^{\prime})}
	\cdot
	 ...
	 .
\end{equation}
We flip the limits in the integral over $ \hat{V}_{out}(t^{\prime \prime})$. The commutation of $ \hat{V}_{in}(t_{i}) $ and $ \hat{V}_{out}(t_{i}) $ under time ordering reduces of the Baker-Campbell-Hausdorff-formula (BCH) to:
\begin{equation}
\hat{S}
=T
\left( 
...
\end{equation}
where in the last step in the last step Eq.\enskip\eqref{eq:Omega_in_converg} has been used.
...
\end{equation}
Since a lot of literature around Quantum field theory relate the $ \hat{S} $ operator in term of the Interaction picture, we shall verify a secondary set of relations:
%\begin{subequations}
%\textcolor[RGB]{0,0,204}{
%\begin{align}
...
,
\end{equation}
the notations for $ \hat{\Omega}_{out}(t) $ and $ \hat{\Omega}_{I}(t) $ from  Eq.\enskip\eqref{eq:Omega_out_converg} and \eqref{eq:Omega_I_Chron_0}\footnote{We are allowed to use here not the full expression for $ \hat{\Omega}_{I}(t) $  given in Eq.\enskip\eqref{Omega_i_complete}, since the goal reads $ \hat{\Omega}_{I}(\infty) $. Only a second term would be carried along that becomes zero when setting $ t \rightarrow +\infty $} lead to:
\begin{equation}
\begin{split}
\hat{S}\  \hat{\Omega}_{out}(t) \hat{\Omega}_{I}(t)^{-1}
&=
...
% \theta(-t)
 ...
 \end{split}
\end{equation}
Following the hermitian conjugate of Eq.\enskip\eqref{barT_to_T}, we proceed similar to what was done below this equation. Deriving at:
\begin{equation}
\begin{split}
\hat{S}\  \hat{\Omega}_{out}(t) \hat{\Omega}_{I}(t)^{-1}
&=\hat{S}\ 
\bar{T}\left(
e^{
-i\int_{\infty}^{t}\mathrm{d}t^{\prime \prime} \hat{V}_{out}(t^{\prime \prime})
+i\int_{0}^{t}\mathrm{d}t^{\prime} \hat{V}_{I}(t^{\prime})}
\right)
\\
&=
\hat{S}\ 
...
&=
\hat{S}\ 
 \hat{\Omega}_{I}(\infty)^{-1}
 .
 \end{split}
\end{equation}
The second equality given in Eq.\enskip\eqref{second_set_S_relations} can be verified starting with the left hand side in the same fashion as before:
\begin{subequations}
\begin{align}
	\hat{\Omega}_{in}(t)\hat{\Omega}_{I}(t)^{-1}&=
T\left( e^{-i\int_{-\infty}^{t}\mathrm{d}t^{\prime} \hat{V}_{in}(t^{\prime})} \right)
...
\hat{S}.}
\end{equation}
This equality is essential in our way of establishing the Gell-Mann Low formula, which is carried out in the next section. First it must satisfy the differential equation for $ \hat{\Omega}_{in}(t) $ given in Eq.\enskip\eqref{eq:time_evo_Omega_in}:
\begin{subequations}
\begin{align}
i\partial_{t}\hat{\Omega}_{in}(t)
	...
	\sum_{n} i^{n}
		...
		\hat{S}.
\end{align}
\text{Now a partial differentiation respect to $ t $ is required:}
\begin{align}
	i\partial_{t}\hat{\Omega}_{in}(t)
	&= i\left( 
	\sum_{n} i^{n}
...
since $ t $ being the earliest time in the integral and anti-chronological time ordering can be applied we are allowed to move $ \hat{V}_{in}(t) $ to the left and out of $ \bar{T} $.
\begin{equation}
\begin{split}
	i\partial_{t}\hat{\Omega}_{in}(t)
	&=
