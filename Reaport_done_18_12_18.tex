...
\end{equation}
We will verify explicitly that Eq.\enskip\eqref{S_defi} is  time independent. For this we insert the expressions given in Eq.\enskip\eqref{eq:Omega_in_converg} and \eqref{eq:Omega_out_converg},
...
\end{split}
\end{equation}
%Since a lot of literature around quantum field theory relate the $ \hat{S} $ operator in term of the Interaction picture, we shall verify a secondary set of relations. Starting in a state in the Interaction picture, we go to the Heisenberg and then to the $ in $ picture similar to what we have done in Eq.\enskip
%\begin{subequations}
%\textcolor[RGB]{0,0,204}{
%\begin{align}
