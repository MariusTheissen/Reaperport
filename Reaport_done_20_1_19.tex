...
In chapter \ref{Pictures in Quantum Mechanics} we discuss the more common Schrödinger, Heisenberg and Interaction picture. Followed up by $ in $ and $ out $ pictures to which Gell-Mann Low formula allows easy transition. These pictures are based on boundary conditions, which are motivated in chapter \ref{in_out_picture_external_currents}, imposing free motion on particles along them. We derive them from experimental and physical points of view. After deriving concrete solutions for the unitary operators associated with these new pictures, we define the scattering operator $ \hat{S} $ in chapter \ref{Scattering operator}. Finally resulting in the  Gell-Mann Low formula.
...
\right) .
\end{equation}
The trivial case $ \hat{Q}_{H}(t) = T \left[\hat{Q}_{H}(t)\right] $ allows us to apply the chronological time ordering to the entire right-hand side.
\begin{equation}\label{1.5st_expression_for_Omega_H}
\hat{Q}_{H}(t)
=
T\left[
\hat{S}^{-1} 
...
.
\end{equation}
In the next step we can move $ \hat{S}^{-1} 
 $ out of the exterior time ordering. This is allowed since it fulfils the conditions of time independence and the key point at this moment: it has \underline{not} been involved or is a result of commutations, that are only equal to $ 1 $ due to the now present exterior time ordering. Therefore we write:
 \begin{equation}\label{1.7st_expression_for_Omega_H}
\hat{Q}_{H}(t)
=
\hat{S}^{-1} 
T\left[
...
.
\end{equation}
As explained in the appendix \ref{chronological_time} inside of time ordering operator commutators can be kept to $ 0 $. We will use it to move $ \hat{Q}_{in}(t)
 $ to
\begin{equation}\label{2th_expression_for_Omega_H}
\hat{Q}_{H}(t)
=
\hat{S}^{-1} 
T\left[
...
.
\end{equation}
To fuse the two inner $ T $ operators, we need to establish under which conditions operators can be moved inside of an already time ordered expression.
Let's consider a case with an arbitrary operator $ \hat{M}(t) $:
  \begin{equation}
%T\left[
%%
\hat{M}(t)
...
.
\end{equation}
We use the non compact and non symbolic expression for $ \hat{\Omega}_{in}(t)
 $ similar to Eq.\enskip\eqref{Omega_I_long_timeorderd}. Focused on the term for $ n =2 $, we write 
\begin{equation}
   \begin{split}
%&T\left[
%%
...
.
   \end{split}
\end{equation}
We see that this product has the operator $ \hat{M}(t) $ already at the chronological time ordered position, because the variable $ t $ is fixed as the latest time by the integral limits.

This can be formulated into a condition. Any operator $ \hat{M}(t) $  multiplied from the left (right) can be moved into a chronological time ordered product dependent on a number of $ t_{i} $ variables , if it can be guaranteed that $ t< t_{i} $ ($ t> t_{i}  $). Thus the correct time ordered position of $ \hat{M}(t) $ is at the head or tail of the set.
...
.
   \end{split}
\end{equation}
In the last step the BCH-formula has been used and due to the chronological time ordering, we could set the appearing commutator to zero.

By taken all step together we can write Eq.\enskip\eqref{1st_expression_for_Omega_H} in the form:
...
\end{split}
\end{equation}
Over the entire product all we can cancel $ \hat{S} $ and $ \hat{S}^{-1} $ by moving the inverted scattering operator into the inner time ordering on their left side. This is allowed due to them being time independent and the exterior $ T $ operator. Reducing the expression to
\begin{equation}
\begin{split}
T\left[
\prod_{j}^{n}
...
\end{split}
\end{equation}
In the second line we moved the $ \hat{S}^{-1} $ operator out of the exterior time ordering same as in Eq.\enskip\eqref{1.5st_expression_for_Omega_H}
. The inner $ T $ operator can be dropped as due to the overall chronological time ordering.  
...
\end{split}
\end{equation}
The scattering operator allows to transform these asymptotic states like:
\begin{equation}\label{S_on_state}
\begin{split}
\bra{0_{out}}
\hat{S}^{-1}
=
\frac{\bra{0_{in}}}{\braket{0_{out}|0_{in}}}
\end{split},
\end{equation}
where the nominator is equal to $ 1 $ for stable vacua. Substituting Eq.\enskip\eqref{S_on_state} in \eqref{GML_applied_0.1} gives
...
\end{split}
\end{equation}
This exponential function can now be treated in a pertubation using the Wick Theorem. 
