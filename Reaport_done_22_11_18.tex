...
\textcolor[RGB]{0,0,204}{
\Phi (\underline{x},t)= \sum_{\underline{k}} q_{\underline{k}}(t)u_{\underline{k}}(\underline{x})
 .}
\end{equation}
...
\end{equation}
Where the volume $ V $ provides normalization. We remark that $ u_{\underline{k}} $ constitutes an orthonormalized basis in the Hilbertspace
...
\begin{equation}
 ...
 \left[ 
	...
	-(i)^{2}\underline{k}^{2}q_{\underline{k}}(t)u_{\underline{k}}(\underline{x})
	+m^{2}q_{\underline{k}}(t)u_{\underline{k}}(\underline{x})
 \right] 
  =j(\underline{x},t).
\end{equation}
To get an equation for $ q_{\underline{k}} $ alone we need to get rid of $ u_{\underline{k}} $ and remove the space dependency in the current. Multiplying with $ u^{\ast}_{\underline{k}'} $ and integrating over the whole space will form the completeness relation on the left-hand side and the Fourier transformation for the current.
\begin{equation}
\int d^{3}\underline{x} \ 
u^{\ast}_{\underline{k}'}
\ \cdot
...
\end{equation}
Using the Eq.\eqref{ortho_relation}
\begin{equation}
 \sum_{\underline{k}}
 \delta_{kk'}
 \left[ 
