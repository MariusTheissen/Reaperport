...
To proceed, we quantize our field in a box of volume $ V $ and length $ L $. The classical field and its canonical momentum $ \Pi = \dfrac{\partial \mathcal{L}}{\partial\dot{\Phi}(\underline{x},t)}=\dot{\Phi}(\underline{x},t) $ are then promoted to operators $ \hat{\Phi}(\underline{x},t) $ and  $ \hat{\Pi}(\underline{x},t) $ in the Heisenberg picture. Satisfying the equal-time commutator:
\begin{equation}
\left[
...
\end{equation}
as a consequence for  Eq.\enskip\eqref{eq_for_q} ,this allows us to distinguish between early and late times. For early time  Eq.\enskip\eqref{eq_for_q} approaches  a homogeneous differential equation. We will call its asymptotic solution by $ \hat{q}_{\underline{k}}(t) \rightarrow \hat{q}_{k,in}(t) $. Explicitly, 
...
\begin{equation}\label{q_In_operators}
 ...
 \hat{q}_{\underline{k},in}(t) 
  ...
  \dfrac{1}{2\omega_{\underline{k}}}\left(
	...
	e^{i\omega_{\underline{k}}t}
  \right) 
  , \leftrightarrow \enskip t\rightarrow -\infty,}
\end{equation}
...
At late times Eq.\enskip\eqref{eq_for_q} also reduces to a homogeneous type. In this case asymptotic the solution $ \hat{q}_{\underline{k}}(t) \rightarrow \hat{q}_{k,out}(t) $ reads
...
After splitting the sinus function, we find that
\begin{equation}
\hat{q}_{\underline{k},out}(t) 
  =
  \hat{q}_{\underline{k},in}(t) 
...
\end{equation}
where $ \bar{j_{\underline{k}}}(\omega_{\underline{k}})= \int^{\infty}_{-\infty}dt \tilde{j_{\underline{k}}}(t) e^{i\omega_{\underline{k}}t} $.
...
These results were all derived and described in the Heisenberg picture. We kept using the full Lagrangian and found a general solution in Eq.\enskip\eqref{q_full}. From this we can proceed as shown in section \textbf{2.2}.  
We split the Hamiltonian as done there: $ \hat{H}= \hat{H_0} +V $. 
\begin{equation}
\hat{H}_{0}({\Phi},{\Pi})=\frac{1}{2}\hat{\Pi}^{2} + \frac{1}{2}(\nabla \hat{ \Phi})^{2} 
+\frac{1}{2}m^{2}\hat{\Phi}^{2},
\end{equation}
\begin{equation}
\hat{V}(\Phi)=j\hat{\Phi}.
\end{equation}
Expressing both operators in terms of the Fourier basis given \eqref{box_quanta}, and the orthonormality relations [Eq.\enskip\eqref{ortho_relation},\eqref{completness_relation}], we can express the Hamiltonian as follows:
\begin{equation}
\hat{H}_{0}({q},\dot{{q}})=
\sum_{\underline{k}}
...
\end{equation}
From this form we go to the Interaction picture. The potential $ V_{I} $ appearing in Eq.\enskip\eqref{eq:time_evo_Omega_interaction} reads :
\begin{equation}
\hat{V}_{I}(q)=
\sum_{\underline{k}}
\tilde{j}(\underline{k},t)\hat{q}_{\underline{k}_{I}}(t),
\end{equation}
where we used Eq.\enskip\eqref{eq:operator_interac_heisenberg} to transform $ \hat{q}_{\underline{k}}(t) $:
\begin{equation}
...
\hat{\Omega}_{I}^{-1}(t).
\end{equation}
But to have a well defined operator $ \hat{\Omega}(t) $  we need conditions for $ \Omega \rightarrow 1 $ which is mostly depended on the current $ j $. There are $ 3 $ possibilities. Ether at $ t = \pm \infty $ or $ t=0 $. This leads then to $ 3 $ different pictures.
%
%
%
%%
...
%\end{equation}
%%This means 
The early time condition defines the $ In $ picture in reminiscence to the first asymptotic solution Eq\enskip\eqref{q_In_operators} and it  writes :
