...
\begin{subequations}\label{eq:time_evo_Omega_interaction}
\textcolor[RGB]{0,0,204}{
\begin{align}
	i\hbar
	\frac{\partial}{\partial t}
	...
	,
	&\\
\text{\textcolor[RGB]{0,0,0}{requiring } \enskip \enskip}
	\hat{\Omega}_{I}(0)
	&=1
	.	
\end{align}
...
\end{subequations}
With he boundary condition a well defined solution can be found. For $ t $ to be at zero is for convenience sake in the derivation of the solution later. Often a general Ansatz of $ \hat{\Omega}_{I}(t_0) =1 $ with any real number is chosen.
...
.
\end{equation}
Expressing both field operators in terms of the Fourier basis given in \eqref{box_quanta}, and using the orthonormality relation Eq.\enskip\eqref{ortho_relation}, as well as the reality condition of the field $ \hat{q}_{\underline{-k}}(t)=\hat{q}^{\star}_{\underline{k}}(t) $ we can express the Hamiltonian as follows:
\begin{equation}
\hat{H}_{0}({q},\dot{{q}})=
\sum_{\underline{k}}
\left\lbrace 
...
\end{align}
\end{subequations}
The factorial sum over $ n $ approaches the exponential function as seen in calculus and this non perturbative expression reads:
