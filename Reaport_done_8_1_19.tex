...
Quantum field theory is a formalism that allow us to describe the interaction between elementary particles. Of great interest are observable quantities like cross-sections or decay rates, which can be established theoretically and contrasted with experimental results. Both observables can be determined by calculating the amplitude of the regarding process. Such calculations require pertubative approaches. However the starting points for setting up correlations functions like a Lagrangian, operators and states are described and formulated in the way of the Heisenberg picture without any parts that allow for pertubation theory. The main goal of this report is the derivation of the solution to this dilemma. The Gell-Mann Low formula. It allows transition to pictures inside the correlation functions without loss of information by including the scattering operator $ S $. This $ S $ allows to be calculated pertubatively and therefore also the actual correlation functions.  
...
\\
	...
	&=
		-i\dot{\hat{\Omega}}_{out}(t)\hat{\Omega}^{-1}_{out}(t)
\\
%\text{Using Eq.\enskip\eqref{eq:time_evo_Omega_out} $\mid \qquad $}
...
\end{align}
\end{subequations}
One $ i $ taken out of the sum allows us to remove the negative sign and restores the correct power $ n-1 $ to reapplying anti-chronological time ordering:
\begin{equation}
...
\end{split}
\end{equation}
 On the other hand we also need to verify the initial condition $ \hat{\Omega}_{in}(-\infty)= 1 $ still holds. We begin with the explicit expression from Eq.\enskip\eqref{Omeaga_in_for_GML}:
%\begin{subequations}
%\begin{align}
%\hat{\Omega}_{in}(t)
%	&=\bar{T}
...
Keeping this in mind, we recognize the hermitian conjugate of Eq.\enskip\eqref{eq:Omega_in_converg} taking at $ t=\infty $ in Eq.\enskip\eqref{Omega_in_minus_infty_GML}.
\begin{equation}
\begin{split}
\hat{\Omega}_{in}(-\infty)
&= 	(\hat{\Omega}_{in}(\infty))^{\dagger}\hat{S}
...
\end{split}
\end{equation}
This last step required $ \hat{S} $ to be unitary. This can be seen straight away  by taking the hermitian conjugate of Eq.\enskip\eqref{S_defi}.
...
\end{split}
\end{equation}
One can easily derive this expression in terms of the quantized field $\hat{ \Phi }$. It follows from the inversion of Eq.\enskip\eqref{box_quanta}, $ \hat{q}_{\underline{k}}(t) = \int \! dx \  \hat{ \Phi }(\underline{x},t)u^{\star}_{\underline{k}}(\underline{x}) $ and its substitution in Eq.\enskip\eqref{Wick_theorem_relation_scalar_modes}. A repeated use of the completeness relation in Eq.\enskip\eqref{completness_relation} lead us to:% express:% The Green's function becomes the scalar propagator $ \triangle(x-x')$ . Which is the Green's function to the free equation of motion of the real scalar field. An equation similar to \eqref{q_Out_by_q_In} for the fields $ \Phi_{in} $ and $ \Phi_{out} $ can be derived. Here  $ i\triangle(x-x')$ would take the place of the $  \sin
%    \left[
%    \omega_{\underline{k}}(t-t') 
...
\right) .
\end{equation}
The chronological time ordering on both sides of $ Q_{in}(t) $ can be expanded.
\begin{equation}
\begin{split}
Q_{H}(t)
=
...
\end{split}
\end{equation}
reapplying one total chronological time ordering to both brackets allows us to moves $ Q_{in}(t)
 $to the very left. But it must stay inside this new $ T $ alongside all the integrals because the correct time ordered position for $ Q_{in}(t)
 $ is just between the limits $ \pm \infty $ of the total integral. This rearrangement inside of $ T $ is due to commutation inside of it and is explained in the  appendix\ref{chronological_time}. Compactly written:
% \begin{equation}
%\begin{split}
%Q_{H}(t)
%=
...
\end{split}
\end{equation}
Using the BCH-formula here, we find an expression for $ \hat{S} $ given in the second line of Eq.\enskip\eqref{S_first_3}. 
 \begin{equation}
\begin{split}
Q_{H}(t)
=
...
\end{split}
\end{equation}
It must be stressed that this only holds for $ \hat{S} $ inside of the $ T $ operator. As we remarked $ Q_{in}(t) $ should be written between the exponential functions in expression prior.
