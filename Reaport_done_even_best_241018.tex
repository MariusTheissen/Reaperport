...
By working out a few handy techniques and methods on the way, we will proof the Gell-Mann Low formula.\textcolor{red}{What is the GellMann Law formula, why is it so important explain in worth}
...
\end{equation}
with $ \hat{U} = e^{-\frac{i}{\hbar}\hat{H}(t-t_{0})} $ is the time evolution operator, which satisfies the differential equation $i\hbar\partial_{t}\hat{U}=\hat{H}\hat{U}$.  Here $ \ket{\Psi_{S}(t_0)} $ is a ket of $ t=t_{0} $. We shall generally take $ t_{0}=0 $ and write
...
\textcolor[RGB]{0,0,204}{
\Phi(x) = \Phi (\pmb{x},t)= \sum_{\pmb{k}} q_{\pmb{k}}(t)u_{\pmb{k}}(\pmb{x})
 .}
\end{equation}
In this separated time and space dependency, we choose the Fourier basis for $ u_{\pmb{k}} $. 
\begin{equation}\label{fourierbasis}
u_{\pmb{k}}(\pmb{x})
=
\dfrac{1}{\sqrt{V}} e^{i\pmb{k}\pmb{x}}
\end{equation}
The normalisation over the volume $ V $ comes form the box we impose for the field. In addition $ u_{\pmb{k}} $ fulfills the completeness relation.
...
\partial^{2}+m^{2}
 \right)
 \sum_{\pmb{k}} q_{\pmb{k}}(t)u_{\pmb{k}}(\pmb{x})
 =j(\pmb{x},t)
	&\\
 \sum_{\pmb{k}}
 \left[ 
	...
	+m^{2}
	\right)  
	q_{\pmb{k}}(t)u_{\pmb{k}}(\pmb{x})
 \right] 
 =j(\pmb{x},t).
\end{align}
\end{subequations}
The basis for $ u_{\pmb{k}} $ will enable us to take the spacial derivative.
\begin{equation}
 \sum_{\pmb{k}}
 \left[ 
	\ddot{q}_{\pmb{k}}(t)u_{\pmb{k}}(\pmb{x})
	-(i)^{2}\pmb{k}^{2}q_{\pmb{k}}(t)u_{\pmb{k}}(\pmb{x})
	+m^{2}q_{\pmb{k}}(t)u_{\pmb{k}}(\pmb{x})
 \right] 
  =j(\pmb{x},t).
\end{equation}
To get an equation for $ q_{\pmb{k}} $ alone we need to get rid of $ u_{\pmb{k}} $ and remove the space dependency in the current. Multiplying with $ u^{\ast}_{\pmb{k}'} $ and integrating over the whole space will form the completeness relation on the left-hand side and the Fourier transformation for the current.
\begin{equation}
\int d^{3}\pmb{x} \ 
u^{\ast}_{\pmb{k}'}
\ \cdot
\vert 
 ...
 =
 \int d^{3}\pmb{x} \ 
j(\pmb{x},t) \dfrac{1}{\sqrt{V}} e^{i\pmb{k}\pmb{x}}
\end{equation}
\begin{equation}
 \sum_{\pmb{k}}
 \left[ 
 \int d^{3}\pmb{x} \ u^{\ast}_{\pmb{k}'}u_{\pmb{k}}
 \left( 
 \ddot{q}_{\pmb{k}}(t) 
 +
 \left( \pmb{k}^{2}+m^{2}\right) 
 q_{\pmb{k}}(t) 
 \right) 
  \right] 
  =\tilde{j}(\pmb{k}',t).
...
\begin{equation}
 ...
 q_{\pmb{k}}(t) 
  ...
  =\tilde{j}(\pmb{k}',t).
\end{equation}
The Kronecker-delta reduces the sum to one term. With change of variable we set choose $ \pmb{k} $ and we define $ (\pmb{k}^{2}+m^{2}) = \omega_{\pmb{k}}^{2} $.
...
%\begin{equation}\label{box_quanta}
%\textcolor[RGB]{0,0,204}{
%\Phi(x) = \Phi (\pmb{x},t)= \sum_{k} q_{k}(t)u_{k}(\pmb{x})
% .}
%\end{equation}
...
% \sum_{k}
% \left[ 
%	\ddot{q}_{k}(t)u_{k}(\pmb{x})
%	-(i)^{2}k^{2}q_{k}(t)u_{k}(\pmb{x})
%	+m^{2}q_{k}(t)u_{k}(\pmb{x})
% \right] 
%  =j(\pmb{x},t).
%\end{equation}
...
% =
% \int d^{3}\pmb{x} \ 
%j(\pmb{x},t) \dfrac{1}{\sqrt{V}} e^{ik\pmb{x}}
%\end{equation}
%\begin{equation}
% \sum_{k}
% \left[ 
% \int d^{3}\pmb{x} \ u^{\ast}_{k'}u_{k}
% \left( 
% \ddot{q}_{k}(t) 
% +
% \left( k^{2}+m^{2}\right) 
