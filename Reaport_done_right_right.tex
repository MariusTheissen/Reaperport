...
As a motivation for this section, consider the set-up of most experiments especially in elementary particle and nuclear physics. Several particles approach each other from a microscopical scale, interact in a microscopical section. Afterwards, the products of the interaction spread again on a microscopical level. Therefore, a good description can be derived by taking the states of the system at the beginning and end as direct products of single particle effectively non-interacting states.\\ 
To bring this concept on into our formulation let's consider $ V=j(t)\hat{q}(t) $ with $ \hat{q}(t) $ as the operator of position and $ j $ an external source.\\
This term will lead to a Poissonian probability distribution for the excitation of states, especially the vacuum or ground states of a system\footnote{see appendix}. Resulting in an unstable vacuum. But for experiments and calculations we have asymptotic states, meaning we can distinguish between states and/or particles. This fits the observation of a stable vacuum without external influences. With asymptotic conditions on $ j(t) $ . We assume it vanishes outside of finite intervals. 
\begin{equation}
\textcolor[RGB]{0,0,204}{
...
\end{equation}
%This means 
The early time condition defines the In picture and it  writes :
...
An unstable vacuum is characterised as vacuum having a finite probability, different from zero, to emit particles.\\
...
\end{split}
\end{equation}
Using this, we obtain first of all the probability for staying in the ground state:
\begin{subequations}
\begin{align}
p_{0}&=\left| \braket{0_{Out}|0_{In}}\right|^{2}=\left| \braket{0_{In}|S|0_{In}}\right|^{2}
	&\\
	&= 	exp\left\lbrace -\int \dfrac{d^{3}k}{(2\pi)^{3}}\dfrac{1}{2\omega} \ 
 		 			 |J(k)|^{2}
 		 			 \right\rbrace .
\end{align}
\end{subequations}
This confirms our statement about an unstable vacuum. The negative sign in the exponent translates to smaller probability at higher higher external currents. So the vacuum can change it's state if a current is $ >0 $.
