...
\end{equation}
The subscript $ S $ represents the object in the Schrödinger picture. Alternatively we may pass to Heisenberg picture, in which the operators carry the time evolution. The relationship between the two pictures is established by Eq. \eqref{eq:matrix_ele_time_dep}.\\
Indeed, the Heisenberg operator defined according to
...
\end{equation}
its time evolution is dictated by an equation that follows from differentiating the equation above and reads
\begin{equation}
