...
\end{equation}
with $ \hat{U} = e^{-\frac{i}{\hbar}\hat{H}(t-t_{0})} $ denoting the time evolution operator. Here $ \ket{\Psi_{S}(t_0)} $ is a ket of $ t=t_{0} $. We shall generally take $ t_{0}=0 $ and write
\begin{equation}
\textcolor[RGB]{0,0,204}{
\ket{\Psi_{S}(t)} 
...
\end{equation}
The state one the right-hand side now has no more time dependence. This turns it in contrast to Eq.\eqref{eq:time_ev_op_schr} into a state in a new representation called the Heisenberg picture.\\
These two pictures differ by the way of storing the time dependence of the total system. In Schrödinger the states carried it, leaving for the Heisenberg picture only operators as a possibility. To perform this transition of dependence we look at matrix elements of operators are given by 
...
\end{equation}
This new operator $ \hat{A}_{H} $ in combination with the state $ \Psi_{H} $ build the set to work in the Heisenberg picture.
The choice of a picture always requires the transition of states as well as operators. 
Now its time evolution is dictated by an equation that follows from differentiating the equation above and reads
\begin{equation}
...
%e^{-\frac{i}{\hbar}t \hat{H}}.
\end{equation}
Here we used the derivative of the unitary operator itself:
\begin{equation}\label{eq:time_evo_unitary}
\textcolor[RGB]{0,0,204}{
\frac{i}{\hbar}\partial_{t}\hat{U}=\hat{H}\hat{U}
.
}
\end{equation}
\begin{subequations}
...
\frac{d}{dt}\hat{A}_{H}(t)
	...
	&\\
	&=\frac{i}{\hbar}
	\hat{U}^{-1}\hat{H}\hat{U}\hat{U}^{-1}\hat{A}_{S}\hat{U}
	-
	\frac{i}{\hbar}
	\hat{U}^{-1}\hat{A}_{S}\hat{U}\hat{U}^{-1}\hat{H}\hat{U}.
\end{align}
\end{subequations}
The inserted $ 1 $ allows us to transform the operators from Schrödinger picture to Heisenberg, including The Hamiltonian.
\begin{equation}
\frac{d}{dt}\hat{A}_{H}(t)
	=\frac{i}{\hbar}
	\hat{H}(t)_{H}\hat{A}(t)_{H}
	-
	\frac{i}{\hbar}
	\hat{A}(t)_{H}\hat{H}(t)_{H}
.
\end{equation} 
Giving us:
...
\frac{-i}{\hbar}\left[ \hat{A}_{H}(t),\hat{H}_{H}(t)\right] 
.
}
\end{equation} 
\subsection{Interaction picture}
The third picture is called the Interaction picture (sometimes called the Dirac picture). In it the state and any operator has a time dependence.\\
Suppose the Hamiltonian from the Schrödinger picture  $ \hat{H} = \hat{H}_{0}+V$. Normally the $ \hat{H}_{0} $ represents the free motion of a particle. $ V $ represents in general an external influence or new term complicating a basic problem. Although it often used in a perturbative approach the Interaction picture does not require $ V $  to be small or of any special form.\\
Inserting this new $ \hat{H} $ in the unitary operator introduced above:
...
\end{equation}
At the level of operators, the connection between Interaction and Heisenberg be established by substituting $ \hat{A}_{S} $ in Eq.\eqref{eq:S-H-Operator_Trafo} into Eq.\eqref{eq:operator_interac_schrodinger}
\begin{subequations}
\begin{align}
	\hat{A}_{I}(t)
	&=
